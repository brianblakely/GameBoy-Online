...
var XAudioJSWebAudioWatchDogTimer;
var XAudioJSWebAudioWatchDogLast = 0;
var XAudioJSWebAudioLaunchedContext = false;
var XAudioJSAudioContextSampleBuffer = new Float32Array(0);
var XAudioJSResampledBuffer = new Float32Array(0);
...
var XAudioJSMediaStreamBuffer = new Float32Array(0);
...
var Resampler = /** @class */ (function () {
    ...
    Resampler.prototype.compileLinearInterpolationFunction = function () {
        ...
        this.resampler = function (buffer) {
            ...
            var outLength = _this.outputBufferSize;
            if (bufferLength % _this.channels == 0) {
                if (bufferLength > 0) {
                    var weight = _this.lastWeight;
                    var firstWeight = 0;
                    var secondWeight = 0;
                    var sourceOffset = 0;
                    var outputOffset = 0;
                    var outputBuffer = _this.outputBuffer;
                    for (; weight < 1; weight += _this.ratioWeight) {
                        secondWeight = weight % 1;
                        firstWeight = 1 - secondWeight;
                        for (var channel = 0; channel < _this.channels; ++channel) {
                            outputBuffer[outputOffset++] =
                                _this.lastOutput[channel] * firstWeight +
                                    buffer[channel] * secondWeight;
                        }
                    }
                    weight -= 1;
                    for (bufferLength -= _this.channels,
                        sourceOffset = Math.floor(weight) * _this.channels; outputOffset < outLength && sourceOffset < bufferLength;) {
                        secondWeight = weight % 1;
                        firstWeight = 1 - secondWeight;
                        for (var channel = 0; channel < _this.channels; ++channel) {
                            outputBuffer[outputOffset++] =
                                buffer[sourceOffset + (channel > 0 ? channel : 0)] *
                                    firstWeight +
                                    buffer[sourceOffset + (_this.channels + channel)] * secondWeight;
                        }
                        weight += _this.ratioWeight;
                        sourceOffset = Math.floor(weight) * _this.channels;
                    }
                    for (var channel = 0; channel < _this.channels; ++channel) {
        ...
        this.resampler = function (buffer) {
            ...
            if (bufferLength % _this.channels == 0) {
                if (bufferLength > 0) {
                    ...
                    var amountToNext = 0;
                    var alreadyProcessedTail = !_this.tailExists;
                    _this.tailExists = false;
                    var outputBuffer = _this.outputBuffer;
                    var outputOffset = 0;
                    ...
                    do {
                        if (alreadyProcessedTail) {
                            ...
                            for (channel = 0; channel < _this.channels; ++channel) {
                                output[channel] = _this.lastOutput[channel];
                            }
                        ...
                        if (weight <= 0) {
                            for (channel = 0; channel < _this.channels; ++channel) {
                                outputBuffer[outputOffset++] =
                                    output[channel] / _this.ratioWeight;
                            }
                        }
                        else {
                            _this.lastWeight = weight;
                            for (channel = 0; channel < _this.channels; ++channel) {
                                _this.lastOutput[channel] = output[channel];
                            }
                            _this.tailExists = true;
                            break;
                        }
...
var XAudioJSsourceHandle = (_b = (_a = XAudioJSscriptsHandle[XAudioJSscriptsHandle.length - 1]) === null || _a === void 0 ? void 0 : _a.src, (_b !== null && _b !== void 0 ? _b : document.createElement("script")));
var XAudioServer = /** @class */ (function () {
    function XAudioServer(channels, sampleRate, minBufferSize, maxBufferSize, volume, underRunCallback, failureCallback) {
  ...
  */
    ...
    XAudioServer.prototype.remainingBuffer = function () {
        switch (this.audioType) {
            case 1:
            case 3:
                return (Math.floor((XAudioJSResampledSamplesLeft() *
    ...
    XAudioServer.prototype.initializeWebAudio = function () {
        if (!XAudioJSWebAudioLaunchedContext) {
            try {
                exports.XAudioJSWebAudioContextHandle = new AudioContext(); //Create a system audio context.
            }
            catch (error) {
                exports.XAudioJSWebAudioContextHandle = new window.webkitAudioContext(); //Create a system audio context.
            }
        ...
        XAudioJSWebAudioAudioNode = exports.XAudioJSWebAudioContextHandle.createScriptProcessor(XAudioJSSamplesPerCallback, 0, XAudioJSChannelsAllocated);
        ...
        XAudioJSWebAudioAudioNode.connect(exports.XAudioJSWebAudioContextHandle.destination); //Send and chain the output of the audio manipulation to the system audio output.
        this.resetCallbackAPIAudioBuffer(exports.XAudioJSWebAudioContextHandle.sampleRate);
        this.audioType = 1;
        /*
    ...
    XAudioServer.prototype.initializeResampler = function (sampleRate) {
        ...
        XAudioJSResampleBufferSize = Math.max(XAudioJSMaxBufferSize * Math.ceil(sampleRate / this.XAudioJSSampleRate) +
            XAudioJSChannelsAllocated, XAudioJSSamplesPerCallback * XAudioJSChannelsAllocated);
...
function XAudioJSGenerateFlashSurroundString() {
    ...
    for (var index = 0; index < XAudioJSSamplesPerCallback &&
        XAudioJSResampleBufferStart != XAudioJSResampleBufferEnd; ++index) {
        //Sanitize the buffer:
        XAudioJSBinaryString[XAudioJSTotalSamples++] = String.fromCharCode(((Math.min(Math.max(XAudioJSResampledBuffer[XAudioJSResampleBufferStart++] + 1, 0), 2) *
            0x3fff) |
            0) +
            0x3000);
        XAudioJSBinaryString[XAudioJSTotalSamples++] = String.fromCharCode(((Math.min(Math.max(XAudioJSResampledBuffer[XAudioJSResampleBufferStart++] + 1, 0), 2) *
...
function XAudioJSGenerateFlashStereoString() {
    ...
    for (var index = 0; index < XAudioJSTotalSamples &&
        XAudioJSResampleBufferStart != XAudioJSResampleBufferEnd;) {
        //Sanitize the buffer:
        XAudioJSBinaryString[index++] = String.fromCharCode(((Math.min(Math.max(XAudioJSResampledBuffer[XAudioJSResampleBufferStart++] + 1, 0), 2) *
            0x3fff) |
            0) +
            0x3000);
        XAudioJSBinaryString[index++] = String.fromCharCode(((Math.min(Math.max(XAudioJSResampledBuffer[XAudioJSResampleBufferStart++] + 1, 0), 2) *
...
function XAudioJSResampleRefill() {
    if (XAudioJSAudioBufferSize > 0) {
        ...
        var resampleLength = XAudioJSResampleControl !== null
            ? XAudioJSResampleControl.resampler(XAudioJSGetBufferSamples())
            : 0;
        var resampledResult = XAudioJSResampleControl !== null
...
function XAudioJSResampledSamplesLeft() {
    return ((XAudioJSResampleBufferStart <= XAudioJSResampleBufferEnd
        ? 0
...
var sourceOfWorker = (_d = (_c = scripts[scripts.length - 1]) === null || _c === void 0 ? void 0 : _c.src, (_d !== null && _d !== void 0 ? _d : ""));
var Resize = /** @class */ (function () {
    ...
    Resize.prototype.resizeWidthInterpolatedRGB = function (buffer) {
        ...
        for (var interpolationWidthSourceReadStop = this.widthOriginal - 1; weight < interpolationWidthSourceReadStop; targetPosition += 3, weight += ratioWeight) {
            ...
            firstWeight = 1 - secondWeight;
            //Interpolate:
            for (finalOffset = targetPosition, pixelOffset = Math.floor(weight) * 3; finalOffset < this.widthPassResultSize; pixelOffset += this.originalWidthMultipliedByChannels,
                finalOffset += this.targetWidthMultipliedByChannels) {
        ...
        for (interpolationWidthSourceReadStop =
            this.originalWidthMultipliedByChannels - 3; targetPosition < this.targetWidthMultipliedByChannels; targetPosition += 3) {
    ...
    Resize.prototype.resizeWidthInterpolatedRGBA = function (buffer) {
        ...
        for (var interpolationWidthSourceReadStop = this.widthOriginal - 1; weight < interpolationWidthSourceReadStop; targetPosition += 4, weight += ratioWeight) {
            ...
            firstWeight = 1 - secondWeight;
            //Interpolate:
            for (finalOffset = targetPosition, pixelOffset = Math.floor(weight) * 4; finalOffset < this.widthPassResultSize; pixelOffset += this.originalWidthMultipliedByChannels,
                finalOffset += this.targetWidthMultipliedByChannels) {
                outputBuffer[finalOffset] =
                    buffer[pixelOffset] * firstWeight +
                        buffer[pixelOffset + 4] * secondWeight;
        ...
        //Handle for only one interpolation input being valid for end calculation:
        for (interpolationWidthSourceReadStop =
            this.originalWidthMultipliedByChannels - 4; targetPosition < this.targetWidthMultipliedByChannels; targetPosition += 4) {
            for (finalOffset = targetPosition,
                pixelOffset = interpolationWidthSourceReadStop; finalOffset < this.widthPassResultSize; pixelOffset += this.originalWidthMultipliedByChannels,
                finalOffset += this.targetWidthMultipliedByChannels) {
                outputBuffer[finalOffset] = buffer[pixelOffset];
    ...
    Resize.prototype.initializeFirstPassBuffers = function (BILINEARAlgo) {
        ...
        if (!BILINEARAlgo) {
            this.outputWidthWorkBench = new Float32Array(this.originalHeightMultipliedByChannels);
        }
    };
    Resize.prototype.initializeSecondPassBuffers = function (BILINEARAlgo) {
        ...
        if (!BILINEARAlgo) {
            this.outputHeightWorkBench = new Float32Array(this.targetWidthMultipliedByChannels);
        }
    };
    return Resize;
}());
...
var GameBoyCore = /** @class */ (function () {
    function GameBoyCore(canvas, ROMImage, stringROM) {
        ...
        this.ROM = new Uint8Array(0); //The full ROM file dumped to an array.
        this.memory = new Uint8Array(0); //Main Core Memory
        this.MBCRam = new Uint8Array(0); //Switchable RAM (Used by games for more RAM) for the main memory range 0xA000 - 0xC000.
        this.VRAM = new Uint8Array(0); //Extra VRAM bank for GBC.
        this.GBCMemory = new Uint8Array(0); //GBC main RAM Banks
        ...
        this.channel3PCM = new Int8Array(0); //Channel 3 adjusted sample buffer.
        //Vin Shit:
        ...
        this.lastIteration = new Date().getTime(); //The last time we iterated the main loop.
        ...
        this.cBATT = false; //Does the cartridge have battery save?
        ...
        this.BGCHRBank1 = new Uint8Array(0);
        this.BGCHRBank2 = new Uint8Array(0);
        //Palettes:
        this.colors = [0xefffde, 0xadd794, 0x529273, 0x183442]; //"Classic" GameBoy palette colors.
        this.gbcOBJRawPalette = new Uint8Array(0);
        this.gbcBGRawPalette = new Uint8Array(0);
        this.gbOBJPalette = new Int32Array(0);
        this.gbBGPalette = new Int32Array(0);
        this.gbcOBJPalette = new Int32Array(0);
        this.gbcBGPalette = new Int32Array(0);
        this.gbBGColorizedPalette = new Int32Array(0);
        this.gbOBJColorizedPalette = new Int32Array(0);
        this.cachedBGPaletteConversion = new Int32Array(0);
        this.cachedOBJPaletteConversion = new Int32Array(0);
        this.colorizedGBPalettes = false;
        this.frameBuffer = new Int32Array(0); //The internal frame-buffer.
        ...
        this.OPCODE = [
            ...
            //#0x08:
            function (parentObj) {
                var temp_var = (parentObj.memoryRead((parentObj.programCounter + 1) & 0xffff) << 8) |
            ...
            //#0x09:
            function (parentObj) {
                var dirtySum = parentObj.registersHL +
            ...
            function (parentObj) {
                ...
                if ((parentObj.interruptsEnabled & parentObj.interruptsRequested & 0x1f) >
                    0) {
            ...
            //#0x86:
            function (parentObj) {
                var dirtySum = parentObj.registerA +
                    parentObj.memoryReader[parentObj.registersHL](parentObj, parentObj.registersHL);
                parentObj.FHalfCarry = (dirtySum & 0xf) < (parentObj.registerA & 0xf);
                parentObj.FCarry = dirtySum > 0xff;
                parentObj.registerA = dirtySum & 0xff;
            ...
            function (parentObj) {
                if (!parentObj.FZero) {
                    var temp_pc = (parentObj.memoryRead((parentObj.programCounter + 1) & 0xffff) << 8) |
                        parentObj.memoryReader[parentObj.programCounter](parentObj, parentObj.programCounter);
                    parentObj.programCounter = (parentObj.programCounter + 2) & 0xffff;
                    parentObj.stackPointer = (parentObj.stackPointer - 1) & 0xffff;
            ...
            //#0xE8:
            function (parentObj) {
                var temp_value2 = (parentObj.memoryReader[parentObj.programCounter](parentObj, parentObj.programCounter) <<
                    24) >>
                    24;
                parentObj.programCounter = (parentObj.programCounter + 1) & 0xffff;
            ...
            //#0xEA:
            function (parentObj) {
                parentObj.memoryWrite((parentObj.memoryRead((parentObj.programCounter + 1) & 0xffff) << 8) |
            ...
            function (parentObj) {
                ...
                parentObj.stackPointer = (parentObj.stackPointer - 1) & 0xffff;
                parentObj.memoryWriter[parentObj.stackPointer](parentObj, parentObj.stackPointer, (parentObj.FZero ? 0x80 : 0) |
            ...
            //#0xFA:
            function (parentObj) {
                parentObj.registerA = parentObj.memoryRead((parentObj.memoryRead((parentObj.programCounter + 1) & 0xffff) << 8) |
                    parentObj.memoryReader[parentObj.programCounter](parentObj, parentObj.programCounter));
                parentObj.programCounter = (parentObj.programCounter + 2) & 0xffff;
            },
            //EI
        ...
        this.TICKTable = Uint8Array.from([
        ...
        this.SecondaryTICKTable = Uint8Array.from([
    ...
    GameBoyCore.prototype.saveState = function () {
        var sram = this.MBCRam.slice(0);
        var rtc = this.saveRTCState();
        var saveState = {
            state: [
                this.inBootstrap,
                this.registerA,
    ...
    GameBoyCore.prototype.ROMLoad = function () {
        ...
        this.usedBootROM =
            exports.settings[1] &&
                ((!exports.settings[11] && this.GBCBOOTROM.length == 0x800) ||
                    (exports.settings[11] && this.GBBOOTROM.length == 0x100));
    ...
    GameBoyCore.prototype.interpretCartridge = function () {
        ...
        if (!this.usedBootROM) {
            switch (this.ROM[0x143]) {
                ...
                default:
                    this.cGBC = false;
                    cout("Unknown GameBoy game type code #" +
                        this.ROM[0x143] +
                        ", defaulting to GB mode (Old games don't have a type code).", 1);
            }
    ...
    GameBoyCore.prototype.initializeTiming = function () {
        ...
        this.baseCPUCyclesPerIteration =
            (this.clocksPerSecond / 1000) * exports.settings[6];
    ...
    GameBoyCore.prototype.setupRAM = function () {
        ...
        if (this.numRAMBanks > 0) {
            ...
            //Switched RAM Used
            var MBCRam = typeof this.openMBC == "function"
    ...
    GameBoyCore.prototype.recomputeDimension = function () {
        ...
        this.offscreenWidth = !exports.settings[12] ? 160 : this.canvas.width;
        this.offscreenHeight = !exports.settings[12] ? 144 : this.canvas.height;
    ...
    GameBoyCore.prototype.initLCD = function () {
        ...
        try {
            ...
            var canvasStyle = this.canvas.getAttribute("style");
            if (canvasStyle === null ||
                (canvasStyle !== null && !canvasStyle.includes("image-rendering"))) {
                this.canvas.setAttribute("style", (this.canvas.getAttribute("style") || "") +
                    "; image-rendering: " +
                    (exports.settings[13] ? "auto" : "-webkit-optimize-contrast") +
                    ";" +
                    "; image-rendering: " +
                    (exports.settings[13] ? "auto" : "pixelated") +
                    ";" +
                    "image-rendering: " +
                    (exports.settings[13] ? "optimizeQuality" : "-o-crisp-edges") +
                    ";" +
                    "image-rendering: " +
                    (exports.settings[13] ? "optimizeQuality" : "-moz-crisp-edges") +
                    ";" +
                    "-ms-interpolation-mode: " +
                    (exports.settings[13] ? "bicubic" : "nearest-neighbor") +
                    ";");
            }
    ...
    GameBoyCore.prototype.graphicsBlit = function () {
        if (this.offscreenWidth == this.onscreenWidth &&
    ...
    GameBoyCore.prototype.initSound = function () {
        ...
        if (exports.settings[0]) {
            this.audioHandle = new XAudioServer(2, this.clocksPerSecond / this.audioResamplerFirstPassFactor, 0, Math.max((this.baseCPUCyclesPerIteration * exports.settings[8]) /
                this.audioResamplerFirstPassFactor, 8192) << 1, exports.settings[3], undefined, function () {
                exports.settings[0] = false;
            });
    ...
    GameBoyCore.prototype.changeVolume = function () {
        if (exports.settings[0] && this.audioHandle) {
            this.audioHandle.changeVolume(exports.settings[3]);
        }
    };
    GameBoyCore.prototype.initAudioBuffer = function () {
        ...
        this.bufferContainAmount =
            Math.max((this.baseCPUCyclesPerIteration * exports.settings[7]) /
                this.audioResamplerFirstPassFactor, 4096) << 1;
    ...
    GameBoyCore.prototype.channel1OutputLevelTrimaryCache = function () {
        if (this.channel1CachedDuty[this.channel1DutyTracker] && exports.settings[14][0]) {
    ...
    GameBoyCore.prototype.channel2OutputLevelTrimaryCache = function () {
        if (this.channel2CachedDuty[this.channel2DutyTracker] && exports.settings[14][1]) {
    ...
    GameBoyCore.prototype.channel3OutputLevelSecondaryCache = function () {
        if (this.channel3Enabled && exports.settings[14][2]) {
    ...
    GameBoyCore.prototype.channel4OutputLevelSecondaryCache = function () {
        if (this.channel4Enabled && exports.settings[14][3]) {
                ...
                this.LINECONTROL[143] = function (parentObj) {
                    if (parentObj.LCDTicks < 80) {
                        ...
                        if (parentObj.drewBlank == 0) {
                            ...
                            if (parentObj.totalLinesPassed < 144 ||
    ...
    GameBoyCore.prototype.clockUpdate = function (manualTime) {
        ...
        if (manualTime) {
            var seconds = manualTime.seconds, minutes = manualTime.minutes, hours = manualTime.hours, days = manualTime.days;
    ...
    GameBoyCore.prototype.resizeFrameBuffer = function () {
        ...
        if (this.resizePathClear) {
            ...
            this.resizer !== null && this.resizer.resize(this.swizzledFrame);
        }
    };
    GameBoyCore.prototype.compileResizeFrameBufferFunction = function () {
        if (this.offscreenRGBCount > 0) {
            var parentObj = this;
            this.resizer = new Resize(160, 144, this.offscreenWidth, this.offscreenHeight, false, exports.settings[13], false, function (buffer) {
    ...
    GameBoyCore.prototype.GBCtoGBModeAdjust = function () {
        ...
        this.VRAM = this.GBCMemory = this.BGCHRCurrentBank = this.BGCHRBank2 = new Uint8Array(0);
        this.tileCache.length = 0x700;
        if (exports.settings[4]) {
    ...
    GameBoyCore.prototype.getGBCColor = function () {
        ...
        for (var counter = 0; counter < 4; counter++) {
            ...
            //BG
            this.cachedBGPaletteConversion[counter] = this.RGBTint((this.gbcBGRawPalette[adjustedIndex | 1] << 8) |
                this.gbcBGRawPalette[adjustedIndex]);
            //OBJ 1
            this.cachedOBJPaletteConversion[counter] = this.RGBTint((this.gbcOBJRawPalette[adjustedIndex | 1] << 8) |
    ...
    GameBoyCore.prototype.updateGBCBGPalette = function (index, data) {
        if (this.gbcBGRawPalette[index] != data) {
            ...
            if ((index & 0x06) == 0) {
                ...
                data =
                    0x2000000 |
                        this.RGBTint((this.gbcBGRawPalette[index | 1] << 8) |
                ...
                data = this.RGBTint((this.gbcBGRawPalette[index | 1] << 8) |
                    this.gbcBGRawPalette[index & 0x3e]);
                index >>= 1;
                this.gbcBGPalette[index] = data;
                this.gbcBGPalette[0x20 | index] = 0x1000000 | data;
        ...
        if (this.gbcOBJRawPalette[index] != data) {
            ...
            if ((index & 0x06) > 0) {
                ...
                this.gbcOBJPalette[index >> 1] =
                    0x1000000 |
                        this.RGBTint((this.gbcOBJRawPalette[index | 1] << 8) |
        ...
        if (this.gfxWindowDisplay) {
            ...
            if (scrollYAdjusted >= 0) {
                ...
                if (pixelPosition < pixelPositionEnd) {
                    ...
                    var tileNumber = (this.gfxWindowCHRBankPosition | ((scrollYAdjusted & 0xf8) << 2)) +
                        (this.currentX >> 3);
                    var chrCode = this.BGCHRBank1[tileNumber];
                    if (chrCode < this.gfxBackgroundBankOffset) {
                        chrCode |= 0x100;
        ...
        if (this.gfxSpriteShow) {
            ...
            if (this.gfxSpriteNormalHeight) {
                ...
                for (var length = this.findLowestSpriteDrawable(lineAdjusted, 0x7); spriteCount < length; ++spriteCount) {
                    ...
                    for (currentPixel =
                        this.pixelStart + (linePixel > 8 ? linePixel - 8 : 0); xcoord < xCoordEnd; ++xcoord, ++currentPixel, ++linePixel) {
            ...
            if (this.gfxSpriteNormalHeight) {
                for (; OAMAddress < 0xfea0 && spriteCount < 10; OAMAddress += 4) {
                    ...
                    if ((yoffset & 0x7) == yoffset) {
                        ...
                        tile = this.tileCache[((attrCode & 0x08) << 8) |
                            ((attrCode & 0x60) << 4) |
    ...
    GameBoyCore.prototype.memoryReadJumpCompile = function () {
        ...
        for (var index = 0x0000; index <= 0xffff; index++) {
            if (index < 0x4000) {
                ...
                if ((this.numRAMBanks == 1 / 16 && index < 0xa200) ||
                    this.numRAMBanks >= 1) {
                    if (this.cMBC7) {
                ...
                switch (index) {
                    ...
                    case 0xff02:
                        ...
                        if (this.cGBC) {
                            this.memoryHighReader[0x02] = this.memoryReader[0xff02] = function (parentObj, address) {
                                return ((parentObj.serialTimer <= 0 ? 0x7c : 0xfc) |
                                    parentObj.memory[0xff02]);
                            };
                        }
                        else {
                            this.memoryHighReader[0x02] = this.memoryReader[0xff02] = function (parentObj, address) {
                                return ((parentObj.serialTimer <= 0 ? 0x7e : 0xfe) |
                                    parentObj.memory[0xff02]);
                            };
                        }
                        break;
                    ...
                    case 0xff76:
                        ...
                        this.memoryHighReader[0x76] = this.memoryReader[0xff76] = function (parentObj, address) {
                            parentObj.audioJIT();
                            return ((parentObj.channel2envelopeVolume << 4) |
                    ...
                    case 0xff77:
                        ...
                        this.memoryHighReader[0x77] = this.memoryReader[0xff77] = function (parentObj, address) {
                            parentObj.audioJIT();
                            return ((parentObj.channel4envelopeVolume << 4) |
    ...
    GameBoyCore.prototype.memoryReadMBC = function (parentObj, address) {
        //Switchable RAM
        if (parentObj.MBCRAMBanksEnabled || exports.settings[10]) {
        ...
        if (parentObj.MBCRAMBanksEnabled || exports.settings[10]) {
            switch (parentObj.currMBCRAMBank) {
                ...
                case 0x0b:
                    ...
                    break;
                case 0x0c:
                    return ((parentObj.RTCDayOverFlow ? 0x80 : 0) +
    ...
    GameBoyCore.prototype.registerWriteJumpCompile = function () {
        ...
        this.memoryHighWriter[0x14] = this.memoryWriter[0xff14] = function (parentObj, address, data) {
            if (parentObj.soundMasterEnabled) {
                ...
                if (data > 0x7f) {
                    ...
                    if (parentObj.channel1lastTimeSweep > 0 ||
        ...
        this.memoryHighWriter[0x22] = this.memoryWriter[0xff22] = function (parentObj, address, data) {
            if (parentObj.soundMasterEnabled) {
                ...
                if ((bitWidth == 0x8 && parentObj.channel4BitRange == 0x7fff) ||
    ...
    GameBoyCore.prototype.recompileModelSpecificIOWriteHandling = function () {
        if (this.cGBC) {
            ...
            this.memoryHighWriter[0x41] = this.memoryWriter[0xff41] = function (parentObj, address, data) {
                ...
                parentObj.mode0TriggerSTAT = (data & 0x08) == 0x08;
                parentObj.memory[0xff41] = data & 0x78;
                if ((!parentObj.usedBootROM || !parentObj.usedGBCBootROM) &&
                    parentObj.LCDisOn &&
                    parentObj.modeSTAT < 2) {
                    parentObj.interruptsRequested |= 0x2;
                    parentObj.checkIRQMatching();
            ...
            if (this.cGBC) {
                this.memoryHighWriter[0x6c] = this.memoryWriter[0xff6c] = function (parentObj, address, data) {
                    if (parentObj.inBootstrap) {
                        ...
                        if (parentObj.name + parentObj.gameCode + parentObj.ROM[0x143] ==
                            "Game and Watch 50") {
                            parentObj.cGBC = true;
                            cout("Created a boot exception for Game and Watch Gallery 2 (GBC ID byte is wrong on the cartridge).", 1);
                        }
...
function start(canvas, ROM, stringROM) {
    ...
    exports.gameboy = new GameBoyCore(canvas, ROM, stringROM);
    exports.gameboy.openMBC = openSRAM;
    exports.gameboy.openRTC = openRTC;
    exports.gameboy.start();
    stopped = false;
    run();
}
exports.start = start;
...
function stop() {
    ...
    clearLastEmulation();
    exports.gameboy.canvas.width = exports.gameboy.canvas.width;
    stopped = true;
    return true;
}
exports.stop = stop;
function run() {
    if (GameBoyEmulatorInitialized()) {
        if (!GameBoyEmulatorPlaying()) {
            exports.gameboy.stopEmulator &= 1;
            cout("Starting the iterator.", 0);
            var dateObj = new Date();
            exports.gameboy.firstIteration = dateObj.getTime();
            exports.gameboy.iterations = 0;
            exports.gbRunInterval = setInterval(function () {
                if (!document.hidden) {
                    exports.gameboy.run(false);
                }
            }, exports.settings[6]);
        }
        else {
...
function clearLastEmulation() {
    if (GameBoyEmulatorPlaying()) {
        clearInterval(exports.gbRunInterval);
        exports.gameboy.stopEmulator |= 2;
...
exports.persistValues = {};
function setValue(key, value) {
    exports.persistValues[key] = value;
    exports.saveValue.push(key, value);
}
function deleteValue(key) {
    delete exports.persistValues[key];
    exports.saveValue.push(key, null);
}
function findValue(key) {
    return exports.persistValues[key];
}
exports.saveValue = {
    callbacks: [],
    ...
    if (GameBoyEmulatorInitialized()) {
        try {
            ...
            while (typeof slot === "undefined" &&
                ...
                findValue("FREEZE_" + exports.gameboy.name + "_" + state_suffix) != null) {
                state_suffix++;
            }
            var deviceState = exports.gameboy.saveState();
            setValue("FREEZE_" + exports.gameboy.name + "_" + state_suffix, deviceState.state);
...
function saveSRAM(cacheSRAM) {
    if (GameBoyEmulatorInitialized()) {
        if (exports.gameboy.cBATT) {
            try {
                var sram = cacheSRAM || exports.gameboy.saveSRAMState();
                if (sram.length > 0) {
                    // cout("Saving the SRAM...", 0);
                    setValue("SRAM_" + exports.gameboy.name, sram);
                }
                else {
                ...
                cout('Could not save the current emulation state("' +
                    error.message +
                    '").', 2);
            }
        }
...
function saveRTC(cacheRTC) {
    //Execute this when SRAM is being saved as well.
    if (GameBoyEmulatorInitialized()) {
        if (exports.gameboy.cTIMER) {
            try {
                // cout("Saving the RTC...", 0);
                var rtc = cacheRTC || exports.gameboy.saveRTCState();
                setValue("RTC_" + exports.gameboy.name, rtc);
            }
            catch (error) {
                cout('Could not save the RTC of the current emulation state("' +
                    error.message +
                    '").', 2);
            }
        }
...
function openState(slot, canvas, stringROM) {
    var filename = "FREEZE_" + exports.gameboy.name + "_" + slot;
    try {
        if (findValue(filename) != null) {
            try {
                clearLastEmulation();
                cout("Attempting to run a saved emulation state.", 0);
                var ROM = exports.gameboy.ROM;
                exports.gameboy = new GameBoyCore(canvas, new Uint8Array(0), stringROM);
                exports.gameboy.ROM = ROM;
                exports.gameboy.savedStateFileName = filename;
                exports.gameboy.returnFromState(findValue(filename), false);
                run();
            }
            catch (error) {
                console.error(error);
...
function initNewCanvas() {
    if (GameBoyEmulatorInitialized()) {
        exports.gameboy.canvas.width = !exports.settings[12] ? 160 : exports.gameboy.canvas.clientWidth;
        exports.gameboy.canvas.height = !exports.settings[12] ? 144 : exports.gameboy.canvas.clientHeight;
    }
}
//Call this when resizing the canvas:
function initNewCanvasSize() {
    if (GameBoyEmulatorInitialized()) {
        if (!exports.settings[12]) {
            if (exports.gameboy.onscreenWidth != 160 || exports.gameboy.onscreenHeight != 144) {
                exports.gameboy.initLCD();
            }
        }
        else {
            if (exports.gameboy.onscreenWidth != exports.gameboy.canvas.clientWidth ||
                exports.gameboy.onscreenHeight != exports.gameboy.canvas.clientHeight) {
                exports.gameboy.initLCD();
            }
        }
    }
